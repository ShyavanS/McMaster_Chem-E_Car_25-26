...
namespace encoder
{
  ...
  Encoder::Capture::Capture(int32_t count, int32_t delta, float frequency, float counts_per_rev)
      : captured_count(count), captured_delta(delta), captured_frequency(frequency), counts_per_rev(MAX(counts_per_rev, FLT_EPSILON))
  { // Clamp counts_per_rev to avoid potential NaN
  }
  ...
  Encoder::Encoder(PIO pio, uint sm, const pin_pair &pins, uint common_pin, Direction direction,
      ...
      : pio(pio), sm(sm), enc_pins(pins), enc_common_pin(common_pin), enc_direction(direction), enc_counts_per_rev(MAX(counts_per_rev, FLT_EPSILON)), count_microsteps(count_microsteps), freq_divider(freq_divider), clocks_per_time((float)(clock_get_hz(clk_sys) / (ENC_LOOP_CYCLES * freq_divider)))
  {
  }
